module AdventOfCode.CorruptionChecksum where

-- http://adventofcode.com/2017/day/2


import Data.List.ListUtils (combination)

import Control.Monad.Eff (Eff)
import Control.Monad.Eff.Console (CONSOLE, log, logShow)
import Data.Array (head, last, sort)
...
import Prelude (Unit, discard, map, mod, ($), (-), (/), (<$>), (<*>), (<<<), (==))
...
strArray :: Array (Array String)
strArray = map (split (Pattern "\t")) $ split (Pattern "\n") spreadsheet

intArray :: Array (Array Int)
intArray = map (map $ ceil <<< readInt 10) strArray

intList :: Array (L.List Int)
intList = map L.fromFoldable intArray


resultOne :: Int
resultOne = sum $ map f $ map sort intArray
  where
...
resultTwo = sum $ L.concat $ L.fromFoldable $ map (map calc ) filterd
  where
    ...
    calc _ = 0
    filterd = map (L.filter f) sorted
    f x = 0 == (fromMaybe 1 $ mod <$> L.head x <*> L.last x)
